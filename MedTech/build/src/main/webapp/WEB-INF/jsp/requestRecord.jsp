...
app.controller('myCtrl', function($rootScope) {
	$rootScope.generateHash = function(){
...
//function generateHash() {
	...
	$rootScope.submitRecord = function(){
		$("#successMessage").text("");
		$("#errorMessage").text("");
		var data = new Object();
		data.user = $('#user').val();
		data.hash = $('#hashValue').val();
		data.doctorId = $('#form_doctorLicense').val();
		data.patientSSN = $('#form_patientSSN').val();
		$.ajax({							
			type : "POST",
			url : "${pageContext.request.contextPath}/requestRecord",
			data : {
				"data" : JSON.stringify(data)
			},
			success : function(result) {
				if(result.errorCode==0){
					$("#successMessage").html("Record Requested Successfully.<br> Request transaction Id : "+result.resultObject.transactionID);
		   		}else if(result.errorDetails != null){
		   			$("#errorMessage").text(JSON.stringify(result.errorDetails));
		   			//$rootScope.errorMessage = emessage;
				}else{
...
<li class="col-md-3"><a class="tabactive" href="${pageContext.request.contextPath}/physicianDashboard">Physician Dashboard</a></li>
        ...
        <div class="row">
            ...
            <div class="col-md-12">
             ...
             <div class="form-group" id="hashValueDiv" style="display: none;">
              ...
              <!-- <div class="form-group" id="doctorIdDiv" style="display: none;"> 
               <label>Doctor Id</label>
                <input class="form-control" id="doctorId" placeholder="Enter Doctor Id" type="text">
                </div> -->
